...
if (isset($_POST["submit"])) {
    ...
    $schemeName = mysqli_real_escape_string($conn, $_POST["schemeName"]);
    ...
    if (empty($activityName) || empty($organizingUnit) || empty($schemeName) || empty($yearOfActivity) || empty($teachersParticipated) || empty($studentsParticipated) || empty($file_name)) {
        echo "<div class='alert alert-danger'>All fields are required.</div>";
    } else {
        if (move_uploaded_file($_FILES['file']['tmp_name'], $file_destination)) {
            $sql = "INSERT INTO 92eleven (activityName, organizingUnit, schemeName, yearOfActivity, teachersParticipated, studentsParticipated, file_name) 
                VALUES ('$activityName', '$organizingUnit', '$schemeName', '$yearOfActivity', '$teachersParticipated', '$studentsParticipated', '$file_name')";
...
<body>
    ...
    <div class="container">
        <form action="3.4.4.php" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="schemeName">Name of the Scheme:</label>
                <input type="text" class="form-control" name="schemeName" required>
            </div>
            <div class="form-group">
