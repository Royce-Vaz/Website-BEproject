...
if (isset($_POST["submit"])) {
    ...
    if (empty($titleOfPaper) || empty($authorName) || empty($teacherDepartment) || empty($journalName) || empty($yearOfPublication) || empty($issnNumber)) {
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $file_destination)) {
            $sql = "INSERT INTO 7seventh (titleOfPaper, authorName, teacherDepartment, journalName, yearOfPublication, issnNumber, file_name) VALUES ('$titleOfPaper', '$authorName', '$teacherDepartment', '$journalName', '$yearOfPublication' , '$issnNumber', '$file_name')";
